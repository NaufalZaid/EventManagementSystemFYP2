        ...
        <div class="space-y-4">
            @foreach ($events as $event)
                <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <dl class="mt-4 flex flex-wrap gap-5 text-sm"><div><dt class="text-xs text-slate-500">Capacity</dt><dd class="font-semibold text-slate-800">{{ number_format($event->capacity) }}</dd></div><div><dt class="text-xs text-slate-500">Duration</dt><dd class="font-semibold text-slate-800">{{ $event->duration_minutes / 60 }} {{ Str::plural('hour', $event->duration_minutes / 60) }}</dd></div><div><dt class="text-xs text-slate-500">Hours</dt><dd class="font-semibold {{ $event->is_outside_working_hours ? 'text-amber-700' : 'text-slate-800' }}">{{ $event->is_outside_working_hours ? 'Extended (until 23:00)' : 'Normal (08:00–18:00)' }}</dd></div><div><dt class="text-xs text-slate-500">Committee</dt><dd class="font-semibold text-slate-800">{{ $event->committee ?: 'Not specified' }}</dd></div></dl>
